            ...
            <div class="kc-component-group" data-group="<?= esc($grpKey) ?>">
              ...
              <?php foreach ($groupedCatalog[$grpKey] as $item): ?>
                <?php
                  ...
                  $alias = ($item['alias'] ?? '') . ' ' . ($item['keywords'] ?? '');
                ?>
                <button type="button" class="kc-component" draggable="true" data-component-id="<?= esc($item['id']) ?>" data-group="<?= esc($grpKey) ?>" data-keywords="<?= esc($alias) ?>">
