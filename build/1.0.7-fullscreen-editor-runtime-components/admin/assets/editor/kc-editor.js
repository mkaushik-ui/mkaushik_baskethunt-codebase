/**
 * Knowledge Center enterprise authoring workspace (1.0.7).
 * Editor.js is the document engine; this file owns chrome, readiness, and controls.
 */
(function () {
  'use strict';
  ...
  function showBootError(message) {
    ...
    console.error('[KC Editor]', message);
  }
  ...
  function buildTools() {
    ...
    return { EditorJS: EditorJS, tools: tools };
  }
  ...
  function paintComponents(query) {
    ...
    Object.keys(groups).forEach(function (group) {
      ...
      groups[group].forEach(function (item) {
        ...
        const icon = item.icon || (item.label || '?').charAt(0);
        btn.innerHTML = '<span class="kc-component-ico">' + escapeHtml(icon) + '</span><span><strong>' + escapeHtml(item.label) + '</strong><span>' + escapeHtml(item.description || '') + '</span></span>';
        btn.addEventListener('click', function () {
          if (!ensureReady()) {
            setStatus('Editor is not ready', 'error');
            return;
          }
          insertById(item.id, false);
        });
  ...
  // Immersive shell: hide CMS chrome without Fullscreen API.
